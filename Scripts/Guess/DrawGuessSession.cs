...
internal static class DrawGuessSession
{
    ...
    /// <summary>当前出牌是否为升级版本（由 DrawGuessBlank.OnPlay 在调用前设置）。</summary>
    public static bool IsUpgradedContext { get; set; }

    /// <summary>对局开始时重置（由 Entry.OnRunStarted 调用）。</summary>
    public static void Reset()
    {
        IsUpgradedContext = false;
    }
    ...
    public static async Task RunOwnerAsync(Player owner, uint sessionId)
    {
        ...
        await WaitForOutcomeEchoAsync(owner.NetId, sessionId);
        DrawGuessGrantAction.EnqueueGrant(owner, outcome.CardId, IsUpgradedContext);
