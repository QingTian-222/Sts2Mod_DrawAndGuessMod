...
- 可以开启临摹参考面板，在作画界面右侧搜索卡牌并显示其卡图；点击参考图即可取色，左键设置左键颜色，右键设置右键颜色。
...
- Slay the Spire 2 `0.110.0` 或更高版本。
- [STS2-RitsuLib](https://www.nuget.org/packages/STS2.RitsuLib/) `0.5.0` 或更高版本。
...
- The optional tracing reference panel lets players search for a card, view its art beside the drawing canvas, and sample colors directly from the reference.
...
- The optional tracing reference panel searches for cards and displays their art beside the drawing canvas. Click the reference to sample colors: left-click sets the left color and right-click sets the right color.
...
- *Slay the Spire 2* `0.110.0` or later.
- [STS2-RitsuLib](https://www.nuget.org/packages/STS2.RitsuLib/) `0.5.0` or later.
