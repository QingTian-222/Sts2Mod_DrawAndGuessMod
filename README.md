...
- Basic drawing tools, including a brush, color palette, eraser, and character stamps.
- In singleplayer, the latest 20 complete operations can be undone with the button or `Ctrl+Z`. Undo is disabled in multiplayer.
- Recognition can use either all card pools or only the current character's card pool.
