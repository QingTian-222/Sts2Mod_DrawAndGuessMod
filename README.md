...
- Drawing tools include a brush, fill tool, RGB color picker, independent left/right colors, middle-click eyedropper, and five character stamps. The mouse wheel adjusts brush or stamp size.
- The latest 20 complete operations can be undone in both singleplayer and multiplayer. Use `Ctrl+Z` to undo and `Ctrl+Y` or `Ctrl+Shift+Z` to redo. In multiplayer, each player can undo or redo only their own actions.
...
- Recognition can use either all card pools or only the current character's card pool. Advanced settings can detect loaded pools and disable individual pools from recognition.
