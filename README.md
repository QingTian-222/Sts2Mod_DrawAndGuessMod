...
- 多人协作绘画；只有出牌者选择，最终卡面同步给所有玩家。
- 内置原版 611 张卡牌的预训练特征；设置页可重新预训练当前已安装的全部原版及模组卡牌，并显示实时进度。
...
原版 611 张卡牌的两套特征已随模组发布。启动时直接加载特征表并预热 ONNX 模型；猜测时只需对当前画作运行一次 DINOv2。未包含在发布包中的其他模组卡牌会在运行时提取，也可以提前通过设置页的“预训练当前所有卡牌”统一生成本地缓存。若 ONNX 模型或运行库加载失败，会自动退回轻量分支，不阻断出牌。
