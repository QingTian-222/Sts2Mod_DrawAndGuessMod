...
- 内置原版 621 张卡牌的识别缓存。设置页可以扫描当前安装的原版及模组卡牌、重建本地缓存并显示实时进度。
- 可以选择是否包含多人专属卡牌，默认启用。
...
- Slay the Spire 2 `0.111.0` 测试分支。
- [STS2-RitsuLib](https://www.nuget.org/packages/STS2.RitsuLib/) `0.5.12` 或更高版本。
...
原版 621 张卡牌的两套识别数据已随模组发布。启动时直接加载识别缓存并预热 DINOv2；实验性适配器只在选中对应模式时预热，否则在首次使用时按需加载。猜测时只需对当前画作运行一次 DINOv2。未包含在发布包中的其他模组卡牌会在运行时分析，也可以提前通过设置页的“扫描卡牌并建立识别缓存”统一生成本地缓存。
...
- Recognition caches for all 621 base-game cards are included. The settings page can scan installed base-game and modded cards, rebuild the local cache, and display real-time progress.
...
- Recognition caches for all 621 base-game cards are included. The settings page can scan all currently installed base-game and modded cards, rebuild the local recognition cache, and display progress in real time.
...
- *Slay the Spire 2* `0.111.0` public-beta branch.
- [STS2-RitsuLib](https://www.nuget.org/packages/STS2.RitsuLib/) `0.5.12` or later.
